...
constexpr size_t PACKET_SIZE = 124;
...
inline float altitude(float hPa, float qnh=1013.25f) {
  return hPa > 0 && isfinite(hPa) ? 44330.f*(1-powf(hPa/qnh,0.19029495f)) : NAN;
}
...
inline void encode(uint8_t* b, uint32_t seq, uint32_t ms,
  ...
  u16(b+16,fresh); u16(b+18,0);
  for (int i=0;i<26;++i) f32(b+20+4*i,v[i]);
}
}
